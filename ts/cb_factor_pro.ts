/**
 * 可转债技术面因子(专业版）
 * 获取可转债每日技术面因子数据，用于跟踪可转债当前走势情况，数据由Tushare社区自产，覆盖全历史；输出参数_bfq表示不复权，_qfq表示前复权 _hfq表示后复权，描述中说明了因子的默认传参，如需要特殊参数或者更多因子可以联系管理员评估限量：单次调取最多返回10000条数据，可以通过日期参数循环积分：5000积分每分钟可以请求30次，8000积分以上每分钟500次，具体请参阅积分获取办法
 */
export const CbFactorProApi = "cb_factor_pro"

export interface CbFactorProItem {
  /** 转债代码 */
  ts_code: string;
  /** 交易日期 */
  trade_date: string;
...
export interface CbFactorProParams {
  /** 可转债代码 */
  ts_code?: string;
  /** 开始日期 */
  start_date?: string;
